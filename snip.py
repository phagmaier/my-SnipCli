...
class Database:
    ...
    def search_snippets(self, query: str = "") -> List[Dict]:
        if not query:
            cursor = self.conn.execute("SELECT * FROM snippets ORDER BY modified DESC")
        else:
...
"""
    if open_editor(filepath, template):
        ...
        if content.strip() and len(content) > len(template) * 0.5:
...
class SnippetApp(App):
    ...
    def update_list(self, query: str = ""):
        ...
        for snippet in self.snippets:
            ...
            desc_text = (snippet.get('description') or "").strip()
            if len(desc_text) > 60:
                desc = desc_text[:60] + "..."
            else:
                desc = desc_text
