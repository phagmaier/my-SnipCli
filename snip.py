...
@cli.command()
@click.argument('search_query', nargs=-1, required=False)
def edit(search_query):
    """Edit a snippet's metadata and/or content"""
    db = Database(DB_PATH)
    
    # Search for snippets
    query = " ".join(search_query) if search_query else ""
    ...
    if len(snippets) > 1:
        console.print(f"[cyan]Found {len(snippets)} snippets:[/cyan]\n")
        for i, snippet in enumerate(snippets, 1):
            tags = snippet['tags'].split(',') if snippet['tags'] else []
            tags_str = f"[{', '.join(t.strip() for t in tags)}]"
            console.print(f"{i}. [bold]{snippet['title']}[/bold] {tags_str}")
        
        choice = click.prompt("\nSelect snippet number to edit", type=int)
        if choice < 1 or choice > len(snippets):
    ...
    console.print(f"\n[bold cyan]Editing: {snippet['title']}[/bold cyan]\n")
    
    # Show current values and prompt for changes
    console.print("[dim]Press Enter to keep current value[/dim]\n")
    
    new_title = click.prompt("Title", default=snippet['title'])
    
    current_tags = snippet['tags'].split(',') if snippet['tags'] else []
    tags_display = ','.join(t.strip() for t in current_tags)
    new_tags_input = click.prompt("Tags", default=tags_display)
    new_tags = [t.strip() for t in new_tags_input.split(',') if t.strip()]
    
    current_desc = snippet.get('description', '')
    new_description = click.prompt("Description", default=current_desc)
    
    # Ask if they want to edit the file content
    edit_content = click.confirm("\nEdit file content?", default=False)
    
    # Update database
    db.update_snippet(snippet['id'], new_title, new_tags, new_description)
    ...
    db.close()
    console.print(f"\n[green]✓[/green] Snippet '[bold]{new_title}[/bold]' updated successfully!")


@cli.command()
@click.argument('search_query', nargs=-1, required=False)
def delete(search_query):
    """Delete a snippet"""
    db = Database(DB_PATH)
    ...
    if len(snippets) > 1:
        ...
        choice = click.prompt("\nSelect snippet number to delete", type=int)
        if choice < 1 or choice > len(snippets):
            console.print("[red]Invalid selection[/red]")
            db.close()
            return
    ...
    console.print(f"[bold]{snippet['title']}[/bold]")
    ...
    if filepath.exists():
        try:
            filepath.unlink()
            console.print(f"\n[green]✓[/green] Snippet '[bold]{snippet['title']}[/bold]' deleted successfully!")
        except Exception as e:
            console.print(f"[yellow]Warning: Database entry removed but file deletion failed: {e}[/yellow]")
    else:
        console.print(f"\n[green]✓[/green] Snippet '[bold]{snippet['title']}[/bold]' deleted from database.")
        console.print(f"[dim]Note: File was already missing at {filepath}[/dim]")
...
if __name__ == "__main__":
    ...
    known_commands = ['add', 'import-file', 'list', 'stats', 'dir', 'edit', 'delete']
